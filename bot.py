...
async def safe_edit_message(message, text: str, reply_markup=None) -> None:
    try:
        ...
        if "Message is not modified" in str(e):
            return
        logger.exception("Telegram BadRequest while editing message")
    except Exception:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "today":
        if not state["profile"]["is_complete"]:
            await safe_edit_message(query.message, "Сначала пройди знакомство через /start")
            return
        await safe_edit_message(
            query.message,
            build_daily_message(state["user_tz"], state["profile"]),
    ...
    if query.data == "whoami":
        ...
        await safe_edit_message(query.message, text, reply_markup=main_menu())
        return
    ...
    if query.data == "back_main":
        await safe_edit_message(query.message, "Главное меню:", reply_markup=main_menu())
        return

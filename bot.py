...
HISTORY_BUTTON_FACTS = [
    "В этот день в разные годы происходили научные открытия, политические решения и культурные премьеры, которые потом входили в учебники.",
    "Иногда именно в этот день происходили события, которые сначала казались частными, а позже меняли целые эпохи.",
    "История любит совпадения: в одну и ту же дату могли родиться выдающиеся люди и случиться события мирового масштаба.",
    "Почти каждая дата хранит не один, а сразу несколько исторических слоёв — от личных судеб до больших общественных перемен.",
    "В этот день нередко происходили события, которые позже начинали восприниматься как символ своей эпохи."
]
...
def build_history_today() -> str:
    today = date.today()
    fact = random.choice(HISTORY_BUTTON_FACTS)
    return (
        f"🤔 Что было в этот день в истории\n\n"
        f"Дата: {today.strftime('%d.%m')}\n\n"
        f"{fact}\n\n"
        "История любит совпадения дат: иногда в один и тот же день происходят события совершенно разного масштаба."
    )

...
def build_ai_name_story(name: str) -> str:
    ...
    if not client:
        return (
            ...
            f"Люди с таким именем часто запоминаются не только образом, но и внутренней интонацией. "
            f"Спасибо за веру в Павла Юрьевича. Теперь давай соберём твои данные рождения."
        )
...
- объём: 120–180 слов
- без списков
- не использовать шаблонный универсальный текст, который подходит любому имени
- не подменять имя другим
- если происхождение имени неочевидно, писать аккуратно и без выдумки

В конце обязательно добавь отдельной фразой:
...
"""
    ...
    try:
        ...
        return (
            ...
            f"Часто такие имена ассоциируются с характером, который сочетает чувствительность и внутреннюю опору. "
            f"Спасибо за веру в Павла Юрьевича. Теперь давай соберём твои данные рождения."
        )
...
- Структура: общий фон дня, на что обратить внимание, один практический совет
- Не используй списки
